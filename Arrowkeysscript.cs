...
public class Arrowkeysscript : MonoBehaviour {
    ...
    void UpdateTimeStats() {
     ...
     void UpdateFuel(Vector2 rawinput) {
        ...
        PlayerPrefs.SetFloat("remainingfuel", fuelleft);

    }


    float GetFuel() {
        return PlayerPrefs.GetFloat("remainingfuel");
    }
    ...
    Vector2 Polar(Vector2 rectangular) { 
        float velocity = rectangular.magnitude;
        float angle = Mathf.Atan2(rectangular.y, rectangular.x);
        return new Vector2(velocity, angle);
    }
